...
class StreamListener(tweepy.StreamListener):
    ...
    def on_data(self, data):
        ...
        #Check to see if tweet has a coordinates field
        coord = []
        try:
	...
	a9, f9, utc = -1, -1, -1
        ...
        #Convert twitter date into usable format
...
#        final_step = session.execute_async('insert into ' + self.keyword[1] + '(tweet_id, coordinates, created_at, favorite_count, hashtags, lang, quoted_status_id, retweet_count, tweet_text, is_retweet, user_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', (int(f9),coord,int(numerical_date),b9,hash_tags,c9,d9,e9,text_field,g9,h9))
	final_step = session.execute_async('insert into new' + self.keyword[1] + '(candidate,tweet_id, coordinates, created_at, favorite_count, hashtags, lang, quoted_status_id, retweet_count, tweet_text, is_retweet, user_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', (self.keyword[1],int(f9),coord,int(numerical_date),b9,hash_tags,c9,d9,e9,text_field,g9,h9))
...
candidate_phrases = [list_trump,list_rubio,list_sanders,list_carson,list_cruz,list_clinton,list_kasich,list_bush]

track = ['clinton', 'bush', 'python']

i=0

consumer, consumer_secret, access_token, access_token_secret, key_list = [], [], [], [], []

line_count = 0

#Open file that contains the Twitter API keys
...
#Spawn a new thread with its own API keys, for each of the 8 candidates 
for list_of_phrases in candidate_phrases:

    auth = tweepy.OAuthHandler(consumer[i], consumer_secret[i])
    auth.set_access_token(access_token[i], access_token_secret[i])
    thread = Thread(target=start_streamList, args=(auth, list_of_phrases))
    thread.start()
    i = i + 1
