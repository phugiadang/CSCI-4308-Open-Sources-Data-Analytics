...
#returns polls based on both topic and date
def pollsByTopicDate(name, start, end):
	try:
		start = start.split('-', 2)
		start = date(int(start[0]), int(start[1]), int(start[2]))
		end = end.split('-', 2)
	...
	#create structure we will eventually return
	results = []
	
	#get each chart in the given topic
	charts = pollster.charts(topic = name)
	for chart in charts:
...
def main():
	...
	if(len(sys.argv) == 1):
		...
		pollsByTopicDate(sys.argv[1], sys.argv[2], sys.argv[3])
		print 'Usage: python QueryPolls.py <topic> <start_date> <end_date>'
	...
	# elif((sys.argv[1] == 'topic' or sys.argv[1] == 'Topic') and (sys.argv[2] == 'date' or sys.argv[2] == 'Date')):
	# 	if(len(sys.argv) != 6):
	# 		print 'Usage: python QueryPolls.py topic date <topic> <start_date> <end_date>'
	# 	else:
	# 		pollsByTopicDate(sys.argv[3], sys.argv[4], sys.argv[5])
	...
	# 	#sanity check argument length
	# 	if(len(sys.argv) != 4):
	# 		print 'Usage: python QueryPolls.py date <start_date> <end_date>'
	# 		print 'Date format: YYYY-MM-DD'
	# 	else:
